...
from mkv_subtitle_translator.cli import _check_and_upgrade
...
def _mock_response(mocker, tag_name):
    ...
    mocker.patch("mkv_subtitle_translator.cli.urllib.request.urlopen", return_value=mock_resp)


class TestCheckAndUpgrade:
    def test_skips_when_up_to_date(self, mocker):
        _mock_response(mocker, "v0.1.0")
        run = mocker.patch("mkv_subtitle_translator.cli.subprocess.run")
    ...
    def test_attempts_upgrade_when_behind(self, mocker):
        ...
        mocker.patch("mkv_subtitle_translator.cli.subprocess.run")
    ...
    def test_falls_back_to_pip_when_uv_tool_upgrade_fails(self, mocker):
        _mock_response(mocker, "v9.9.9")
        run = mocker.patch(
            "mkv_subtitle_translator.cli.subprocess.run",
    ...
    def test_returns_silently_on_network_failure(self, mocker):
        mocker.patch(
            "mkv_subtitle_translator.cli.urllib.request.urlopen", side_effect=OSError("boom")
        )
        run = mocker.patch("mkv_subtitle_translator.cli.subprocess.run")
        _check_and_upgrade("0.1.0")
        run.assert_not_called()
    ...
    def test_returns_silently_on_rate_limit(self, mocker):
        ...
        mocker.patch("mkv_subtitle_translator.cli.urllib.request.urlopen", side_effect=error)
        run = mocker.patch("mkv_subtitle_translator.cli.subprocess.run")
        _check_and_upgrade("0.1.0")
        run.assert_not_called()
